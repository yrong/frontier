...
import {describeWithFrontier} from './util_deprecated'
import {
...
describeWithFrontier("Frontier RPC (Contract Methods)", `simple-specs.json`, (context) => {
  ...
  it.only("inifinite loop call should return OutOfGas", async function () {
    this.timeout(0);
  ...
  it.only("inifinite loop send with incr should return OutOfGas", async function () {
    this.timeout(0);
